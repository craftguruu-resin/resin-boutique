(function () {
  ...
  function rowUrl(label, val, fit) {
    ...
    return (
      '<label class="vs-muted" style="display:block;font-size:0.78rem;margin-bottom:0.2rem">' +
      esc(label) +
      '</label><div class="vpm-opt-url-row"><input type="url" class="vrm-opt-url" value="' +
      esc(val) +
  ...
  function syncHeroFitButton(fit) {
    var btn = document.getElementById("vrmHeroFitBtn");
    if (!btn) return;
    var IF = window.CraftguruImageFit;
  ...
  function readOptionsFromForm() {
    ...
    var heroFitBtn = document.getElementById("vrmHeroFitBtn");
    if (heroFitBtn) {
