  ...
  function fetchTaxonomy() {
    ...
    if (base) {
      return fetch(base + "/api/catalog/photo-frame-nav", { cache: "no-store" })
        ...
        .then(function (j) {
          if (j && j.ok && j.nav) return j.nav;
          throw new Error("taxonomy api");
        })
        .catch(function () {
          return fetch("photo-frame-nav-default.json", { cache: "no-store" }).then(function (r2) {
            if (!r2.ok) throw new Error("taxonomy");
            return r2.json();
          });
        });
    }
    return fetch("photo-frame-nav-default.json", { cache: "no-store" }).then(function (r) {
