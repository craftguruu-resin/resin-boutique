...
function readDefaultPhotoFrameNavSync() {
  try {
    var raw = fs.readFileSync(photoFrameNavFilePath(), "utf8");
    return JSON.parse(raw);
  } catch (_) {
    return { version: 1, categories: [] };
  }
 ...
 * @param {(err: Error|null, doc: object) => void} cb
 */
function getPhotoFrameNavMerged(cb) {
  var def = readDefaultPhotoFrameNavSync();
  if (!poolMod.isEnabled()) {
    return process.nextTick(function () {
      cb(null, def);
    });
 ...
 * @param {(err?: Error) => void} cb
 */
function savePhotoFrameNav(doc, cb) {
  if (!poolMod.isEnabled()) {
    return process.nextTick(function () {
      cb(new Error("Database not configured"));
    });
  ...
  if (!doc || typeof doc !== "object" || !Array.isArray(doc.categories)) {
    return process.nextTick(function () {
      cb(new Error("Invalid nav: need { categories: [...] }"));
    });
  }
  poolMod
    .getPool()
...
module.exports = {
  ...
  PHOTO_FRAME_NAV_KEY: PHOTO_FRAME_NAV_KEY,
  ...
  readDefaultPhotoFrameNavSync: readDefaultPhotoFrameNavSync,
  getPhotoFrameNavMerged: getPhotoFrameNavMerged,
  savePhotoFrameNav: savePhotoFrameNav,
};
