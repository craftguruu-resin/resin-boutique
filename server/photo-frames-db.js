...
function resolveSkuForInsert(pool, opts, cb) {
  ...
  function tryAuto(skuTry, attempt) {
    ...
    pool
      .query("SELECT 1 FROM photo_frame_products WHERE lower(trim(sku)) = lower(trim($1)) LIMIT 1", [skuTry])
      .then(function (r) {
        if (r.rows.length) {
          var next = "PF-" + crypto.randomBytes(4).toString("hex").toUpperCase();
  ...
  if (norm) {
    return pool
      .query("SELECT 1 FROM photo_frame_products WHERE lower(trim(sku)) = lower(trim($1)) LIMIT 1", [norm])
      .then(function (r) {
        if (r.rows.length) {
  ...
  tryAuto("PF-" + crypto.randomBytes(4).toString("hex").toUpperCase(), 0);
}

function assertSkuUniqueForUpdate(pool, rid, skuNorm, cb) {
  pool
    .query(
      "SELECT 1 FROM photo_frame_products WHERE lower(trim(sku)) = lower(trim($1)) AND id <> $2 LIMIT 1",
      [skuNorm, rid]
    )
    .then(function (r) {
      if (r.rows.length) {
...
function listAll(search, cb) {
  ...
  var sub = String(opts.sub || "")
    ...
    .slice(0, 80);
  var sql =
    "SELECT id, sku, name, description, image_path, note, is_active, price_inr, mrp_inr, options_json, updated_at, base_category_slug, subcategory_slug FROM photo_frame_products";
  var params = [];
  var wh = [];
...
function getByIdForVendor(id, cb) {
  ...
  pool
    .query(
      "SELECT id, sku, name, description, image_path, note, is_active, price_inr, mrp_inr, options_json, base_category_slug, subcategory_slug FROM photo_frame_products WHERE id = $1",
      [rid]
    )
    .then(function (r) {
      if (!r.rows.length) return cb(null, null);
...
function persistImageBuffer(buf, mime, nameHint, cb) {
  var heroDir = mediaPath.photoFrameProductsMediaFsRoot();
  ...
  var relImage = "media/photo-frame-products/" + fileName;
...
function createRow(opts, cb) {
  ...
  function insertWithImage(imagePath) {
    resolveSkuForInsert(pool, opts, function (skuErr, skuFinal) {
      ...
      var newId = "pf-prod--" + crypto.randomBytes(6).toString("hex");
      pool
        .query(
          "INSERT INTO photo_frame_products (id, name, description, image_path, note, is_active, price_inr, mrp_inr, options_json, sku, base_category_slug, subcategory_slug) VALUES ($1, $2, $3, $4, $5, true, $6, $7, $8::jsonb, $9, $10, $11) RETURNING id, sku, name, description, image_path, note, is_active, price_inr, mrp_inr, options_json, base_category_slug, subcategory_slug",
...
function deleteMaterialsByTaxonomySlot(baseSlug, subSlug, cb) {
  ...
  var params;
  if (s) {
    q = "DELETE FROM photo_frame_products WHERE base_category_slug = $1 AND subcategory_slug = $2";
    params = [b, s];
  } else {
    q = "DELETE FROM photo_frame_products WHERE base_category_slug = $1";
    params = [b];
  }
  pool
    .query(q, params)
...
function deleteRow(id, cb) {
  ...
  var rid = String(id || "").trim().slice(0, 120);
  pool
    .query("DELETE FROM photo_frame_products WHERE id = $1 RETURNING id", [rid])
    .then(function (r) {
      if (!r.rowCount) {
...
function setActive(id, isActive, cb) {
  ...
  var rid = String(id || "").trim().slice(0, 120);
  pool
    .query("UPDATE photo_frame_products SET is_active = $2, updated_at = now() WHERE id = $1 RETURNING id", [rid, !!isActive])
    .then(function (r) {
      if (!r.rowCount) {
        throw new Error("Not found");
      }
...
var DEMO_IDS = ["pf-prod--demo-showcase-frame"];
...
function seedMinimalOptions(badge) {
  return {
    ...
    trustBullets: [],
    sizes: [
      { id: "sz-6x8", label: "6 × 8 inch", image: "", priceInr: 899, mrpInr: 1199 },
      { id: "sz-8x10", label: "8 × 10 inch", image: "", priceInr: 1199, mrpInr: 1499 },
    ],
    qtyOptions: [
      { id: "q1", label: "Single", image: "", priceInr: 0, mrpInr: null },
      { id: "q2", label: "Pair", image: "", priceInr: 150, mrpInr: null },
    ],
    colors: [
      { id: "co-ocean", label: "Ocean blue", hex: "#0ea5e9", image: "" },
...
/**
 * Optional demo row for empty databases (ON CONFLICT DO NOTHING). Uses repo hero art as placeholder image.
 * @param {import('pg').Pool} pool
 * @returns {Promise<void>}
 */
function seedDemoPhotoFramesPromise(pool) {
  var demos = [
    {
      id: DEMO_IDS[0],
      sku: "PF-DEMO-FRAME",
      baseCategorySlug: "photo-frame-lines",
      subcategorySlug: "",
      name: "Demo resin photo frame",
      description:
        "Example vendor-managed photo frame listing — replace from the vendor dashboard. Same options model as raw materials: size, colour, and quantity rows can carry their own MRP.",
      note: "Ships with the rest of the Craftguru resin collection.",
      image: "media/photo-frames-hero-home-banner.png",
      price: 899,
      mrp: 1199,
  ...
  return Promise.all(
    demos.map(function (d) {
      return pool.query(
        "INSERT INTO photo_frame_products (id, name, description, image_path, note, is_active, price_inr, mrp_inr, options_json, sku, base_category_slug, subcategory_slug) VALUES ($1, $2, $3, $4, $5, true, $6, $7, $8::jsonb, $9, $10, $11) ON CONFLICT (id) DO NOTHING",
        [
          d.id,
          d.name,
          d.description,
          ...
          d.sku,
          d.baseCategorySlug || "photo-frame-lines",
          d.subcategorySlug || "",
        ]
      );
    })
...
module.exports = {
  ...
  seedDemoPhotoFramesPromise: seedDemoPhotoFramesPromise,
  DEMO_IDS: DEMO_IDS,
};
