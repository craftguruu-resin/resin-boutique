(function () {
  ...
  function patchPdpView(root, m, entries, idx, mainImg, effPrice, effMrp, pct) {
    ...
    if (window.RESIN_WISHLIST && m) {
      ...
      if (w) window.RESIN_WISHLIST.syncButton(w, m.id, "photo_frame");
    }
  }
    ...
    root.addEventListener("click", function (ev) {
      ...
      if (wish) {
        if (window.RESIN_WISHLIST && state.material) {
          ...
          window.RESIN_WISHLIST.toggle(state.material.id, "photo_frame", function () {
            var hb = root.querySelector("#rmPdpWish");
            var hl = root.querySelector("#rmPdpWishLink");
            if (hb) window.RESIN_WISHLIST.syncButton(hb, state.material.id, "photo_frame");
            if (hl) window.RESIN_WISHLIST.syncButton(hl, state.material.id, "photo_frame");
          });
        }
        return;
      }
  ...
  function mountRmPdpShare(root) {
    ...
    var pageUrl;
    try {
      pageUrl = new URL("photo-frame-product.html?id=" + encodeURIComponent(mid), window.location.href).href;
    } catch (_) {
      pageUrl = "photo-frame-product.html?id=" + encodeURIComponent(mid);
    }
  ...
  function mountRmBulkBuy(root) {
    ...
    wrap.innerHTML = WA.pdpButtonHtml({
      id: "rmBulkBuyBtn",
      productName: state.material.name || "Frame",
      productId: mid,
      productUrl: pageUrl,
    });
  }
  ...
  function render() {
    ...
    if (window.RESIN_WISHLIST && state.material) {
      if (wishBtn) window.RESIN_WISHLIST.syncButton(wishBtn, state.material.id, "photo_frame");
      if (wishLink) window.RESIN_WISHLIST.syncButton(wishLink, state.material.id, "photo_frame");
    }
    root.setAttribute("data-pdp-ready", "1");
  }
  ...
  function load() {
    ...
    if (!id) {
      ...
      if (nav0 && window.PfShopNav) {
        window.PfShopNav.mount(nav0, { activeBase: "", activeSub: "" });
      }
      return;
    }
    var path = "/api/catalog/photo-frame-products/" + encodeURIComponent(id);
