...
(function () {
  ...
  function headerOffset() {
    var h = document.querySelector(".site-top");
    return h ? Math.ceil(h.getBoundingClientRect().height) + 12 : 0;
  }

  function scrollToTarget(el) {
    if (!el) return;
    var top = el.getBoundingClientRect().top + (window.scrollY || window.pageYOffset || 0) - headerOffset();
    window.scrollTo({ top: Math.max(0, top), behavior: reduce ? "auto" : "smooth" });
  }

  function wireShopNowButtons() {
    document.querySelectorAll(".hero-shop-now-btn[href^='#']").forEach(function (btn) {
      if (btn.dataset.cgShopNowWired === "1") return;
      btn.dataset.cgShopNowWired = "1";
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", wireShopNowButtons);
  } else {
    wireShopNowButtons();
  }
})();
